...
**【新ルール】ユーザーのメッセージに中国語の単語や表現が混入している場合、それを日本語に翻訳して修正すること**
- 中国語の単語が日本語の文に混ざっている場合（例：「推荐がありますか？」の「推荐」は中国語）、対応する日本語（例：「おすすめ」）に修正する
- 中国語の文全体が入力された場合（例：「请给我菜单」）、対応する日本語（例：「メニューをください」）に翻訳して修正する
- 修正タイプは "grammar" として報告し、説明に「中国語→日本語」と明記する
- このルールは感度設定に関わらず適用する（中国語の混入は常に修正対象とする）
...
- **ユーザーのメッセージに中国語の単語や表現が含まれている場合、それを「学習が必要な語彙」として指摘すること。例：「推荐」→ 日本語では「おすすめ」。中国語の部分を特定し、対応する日本語を提示する。**
- **修正版（2Aの出力）と元のメッセージを比較し、中国語が日本語に翻訳された箇所があれば、その対応関係を報告すること**
- 簡潔に。日本語で出力
...
ルール:
- **【最優先・絶対厳守】AIの返信に既に出現している単語（質問文中の単語も含む）は絶対に提案しないこと。返信内のすべての単語（送り仮名・活用形も含む）をチェックし、1文字でも一致する語彙があれば除外すること。提案すべき語彙がなくなった場合は null を返すこと。**
  **例（失敗ケース）：AIが「週末は何か予定がありますか？」と質問した場合、「週末」「何か」「予定」はすべてAIの返信中に出現しているので提案しない → null を返す。**
  **例（成功ケース）：AIが「週末は何か予定がありますか？」と質問した場合、返信にない単語（例：「映画」「旅行」「買い物」など）のみを提案する。**
  **例（回答文中の動詞も対象）：AIが「公園に行く予定です」と言った場合、「行く」「公園」「予定」はすべて返信中に出現しているので提案しない。AIの返信全体をチェックすること。**
...
- **【出力前の最終チェック】suggestions 配列を出力する前に、各 suggestion の word がアシスタントの返信全文（質問文・回答文すべて）に含まれていないかもう一度確認すること。含まれている単語は必ず削除し、残った単語だけを出力すること。削除後に suggestions が空になったら null を返すこと。**`
...
=== 【絶対厳守】会話の自然な質問は除去しない ===
以下のタイプの文は、学習アドバイスではなく**自然な会話の流れ**です。絶対に除去しないでください：
- **アシスタントが会話を続けるための自然な質問**（例：「何を食べたいですか？」「ラーメン、好きですか？」「メニュー、見ましたか？」「お水はいかがですか？」など）
- **会話の文脈における相手の意見や好みを尋ねる質問**
- **サービス提供時の自然な確認**（例：「お決まりですか？」「こちらでよろしいですか？」）
- **会話のテンポを維持するための相槌や応答**（例：「はい、ありますよ」「そうですよね」など）

これらの文は会話の進行に不可欠であり、教育的な内容ではありません。
...
【重要】type の分類ルール:
- **動詞の活用形・丁寧形（例：「忘れました」「見ます」など、原形から形が変わった動詞）は "grammar" タイプに分類すること。** 「忘れました」は「忘れる」の丁寧過去形であり、単語（word）ではなく文法ルールです。
- 名詞・形容詞の原形や、新出語彙として学習価値のある一般的な単語のみ "word" タイプにする。

【重要】同一文法ルールの重複防止:
- 助詞に関する知識ポイントは「助詞「は」」「助詞「を」」「助詞「が」」のように統一フォーマットを使用すること。「は」単体や「助詞は」など別表記で重複を作らない。
- その他の文法ルールも名称を統一し、意味的に同じルールが複数の知識ポイントとして出力されないようにすること。
}
