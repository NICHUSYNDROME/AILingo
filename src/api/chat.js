...
export async function generateConversationGoal(scenario, language = 'en', proficiencyScore = null, context = {}) {
  ...
  if (proficiencyScore !== null) {
    if (language === 'ja') {
      ...
      if (proficiencyScore < 4) {
        ...
        levelGuidance += '- 【重要】1つの目標は1つの行動のみにしてください。「打招呼并介绍自己」のように複数の行動を1文にまとめないでください。◯「挨拶する」◯「自己紹介する」✕「挨拶して自己紹介する」\n'
  ...
  const systemPrompt = language === 'ja'
    ? 'あなたは会話シーンアシスタントです。ユーザーは日本語を練習しています。選択されたシーンに基づいて、具体的な会話目標を生成してください。中文で返し、複数のサブ目標は改行で区切ってください。\n\n' +
      ...
      '询问今日特色菜\n' +
      '请求开发票\n\n' +
      '重要: 括号内的补充说明、第二人称指示（例：「如果不会可直接说…」「你可以…」）、动作描述（例：「并指菜单」「指着〜说」）、「用日语」「用英语」などの言語指定は絶対に含めないでください。ユーザーは既に日本語練習中なので、目標は純粋な言語行為そのものだけを簡潔に一文で記述してください。1つの目標は必ず1つの行動のみとしてください。\n\n' +
...
export async function checkTaskCompletion(goal, todos, conversationHistory, language = 'en') {
  ...
  const systemPrompt = language === 'ja'
    ? `あなたは会話の進行状況を追跡するアシスタントです。\n` +
      ...
      `TODO リスト:\n${todoListStr}\n\n` +
      `【重要】TODO の内容に基づいて、誰が（学習者かAIか）そのアクションを行うべきかを判断してください。\n` +
      `例：TODOが「问对方的周末计划」の場合、学習者がAIに対して質問した場合のみ完了とみなす。AIが学習者に質問しただけでは完了としない。\n` +
      `例：TODOが「自我介绍」の場合、学習者が自分について話した場合のみ完了とみなす。AIが学習者に質問しただけでは完了としない。\n` +
