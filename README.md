...
| ✍️ **实时拼写纠正** | 字符级 diff 算法高亮显示用户输入中的新增（绿色）、删除（红色删除线）和替换（删除线+绿色）|
...
**调试日志导出**（Electron）：对话结束后，所有 Agent 的请求/响应数据（`window.__aiLogs`）自动写入 `ai-feedback/debug-logs/` 目录，便于离线分析 Agent 行为。启动时自动清理 30 天前的旧日志。
