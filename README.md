...
# 生产构建并手动打包为 macOS 应用
npm run build
bash scripts/manual-pack.sh

# 打包产物位于 release/AILingo.app
# 直接双击即可运行

# 打包说明
# - 自动进行 ad-hoc 签名，确保 macOS 正常运行
# - 包含应用图标 (icon.icns)
# - 如需分发给他人，打包后建议压缩为 ZIP 或 DMG：
#   ditto -c -k --sequesterRsrc --keepParent release/AILingo.app release/AILingo.zip
#   hdiutil create -volname AILingo -srcfolder release/AILingo.app -ov -format UDZO release/AILingo.dmg
```
...
│   ├── main.cjs                        # 主进程（窗口管理、IPC、文件存储）
│   └── preload.cjs                     # 预加载脚本（桥接 API）
│
├── daemon.cjs                          # 守护进程（TTS 代理 + 后端，已由 Electron 主进程内置替代）
├── scripts/                            # 构建脚本
│   └── manual-pack.sh                  # 手动打包 Electron 应用（含 ad-hoc 签名）
│
├── server/                             # 后端服务
│   └── tts-server.cjs                  # TTS 代理服务器（Node.js，可独立运行或内嵌于主进程）
│
...
└── src/                                # 前端源码
    ...
    ├── index.css                       # 全局样式（主题变量）
    │
    ├── api.js                          # DeepSeek API 封装（6 个 Agent + API Key 测试函数）
    │
    ├── config/
...
- **持久化**：开发环境使用 `localStorage`，Electron 环境通过 IPC 存储在主进程管理的 JSON 文件中
- **AI 通信**：直接通过 `fetch` 调用 DeepSeek API，不经过中间服务器
- **TTS 代理**：支持独立运行或内嵌于 Electron 主进程（`electron/main.cjs`），避免 CORS 和 API Key 暴露
